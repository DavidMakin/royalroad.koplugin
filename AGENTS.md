...
- The ribbon is cleared the moment a check is **selected**. Every check entry point routes through `updater.lua` `clearNewBadges(only)` (`only` = optional `fiction_id -> truthy` set, `nil` = all stories) at check start, before any fetch, so the badge acknowledges the user's intent rather than the fetch outcome: `checkSingleStoryForUpdates` (per-story), `performUpdateCheck` ("Update all"), and `batchUpdate` (batch-selected). A failed fetch does **not** restore it. Opening story options does **not** clear it (viewing options is not an acknowledgement).
- It can also be cleared manually without running a check, via the same helper: a `Clear "+N new" badge` button in the story options dialog (`story_detail.lua`) and `Clear new-chapters badge` in the downloads-list hold menu (`downloads_ui.lua`). Both only appear while `unread_new_count > 0`.
