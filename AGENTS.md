...
- Toggle points: story options dialog (`story_detail.lua`, button after the download actions) and the hold menu in the downloads list (`downloads_ui.lua`). Both clear `story.unread_new_count` only when the story becomes **excluded** (so stale "+N new" badges don't persist on skipped stories); un-excluding preserves the badge, since unread chapters are still unacknowledged.
