...
### In-Book Contents (end of `title.xhtml`)

- Every EPUB carries a reader-visible contents list with one link per chapter, built by `_buildContentsBlock` in `epub.lua` from the `toc_entries` list `_buildTocStructures` collects in its chapter loop. `saveAsEPUB` rewrites the whole EPUB on every update, so it is regenerated — and therefore current — each time chapters are added.
- **The contents are appended to `title.xhtml` behind a `page-break-before`, not given a spine item of their own.** Reading order is still cover page, title page, contents, chapters, and the break makes the contents read as their own page — but they add no spine item. This is the whole design constraint, not a detail:
  - KOReader stores reading positions as xpointers into `/body/DocFragment[N]`, where `N` is the **spine index**. Any extra document ahead of the chapters moves every chapter's `N` on by one, so every stored position lands a chapter early — or fails to resolve and drops the reader at the start of the book.
  - Riding along inside the title page, the contents can grow from one page to many as chapters are added without touching a single index. Page count is layout; xpointers don't depend on layout.
- **Do not try to fix a spine shift by rewriting the DocSettings sidecar.** That was tried (`_shiftSidecarForTocPage`, v0.20.0) and lost a write race: KOReader flushed its own in-memory sidecar one second after the plugin's repair and overwrote it, leaving the reader at the start of the book. Verified on a real book — the repaired `last_xpointer` was gone while `percent_finished`, which the plugin never touches, had been recalculated by KOReader. The sidecar belongs to whoever has the document open; never contend for it.
- Reached from KOReader's TOC menu, not by paging: `_buildTocStructures` adds a `Table of Contents` navPoint (NCX), a nav entry (`nav.xhtml`) and an `epub:type="toc"` landmark, all pointing at `title.xhtml#contents`. `nav.xhtml` remains the machine-readable nav document and stays **out** of the spine.
- Any future change that inserts or removes a spine item ahead of the chapters silently moves every reader's position, and there is no safe way to compensate afterwards. Add front matter to `title.xhtml` instead.
